(function (global) {
  ...
  function chartOptions(snapshot, radio, colors, compact) {
    ...
    const base = () => ({
      ...
      tooltip: { trigger: 'axis', renderMode: 'richText', confine: true, backgroundColor: colors.surface, borderColor: colors.border, textStyle: { color: colors.text, fontSize: 12 }, formatter(params) {
        ...
        return clock(params[0].value[0]) + '\n' + params.map(item => item.seriesName + '  ' + number(item.value[1])).join('\n');
      } },
    ...
    return { signal, temperature, traffic };
  }
  ...
  function mergeSnapshot(previous, data) {
    ...
    const result = { ...data };
    for (const [key, limit] of [['signal', 60], ['traffic', 60]]) {
  ...
  global.SimpleAdminMonitor = { chartOptions, number, availableRadioModes, mergeSnapshot };
  ...
  function mount() {
    ...
    const render = (force = true) => {
      ...
      for (const key of ['signal', 'temperature', 'traffic']) {
    ...
    app = global.Vue.createApp({
      data: () => ({ snapshot: { generation: 0, serverTime: Date.now(), signal: [], traffic: [], trafficSummary: {}, mock: false }, radio: 'NR', error: '' }),
      computed: {
      ...
      methods: {
        ...
        async refresh() {
          ...
          try {
            ...
            fetching = false;
            controller = null;
            if (active()) timer = setTimeout(() => this.refresh(), 1000);
          }
        }
      }
    }).mount(element);
