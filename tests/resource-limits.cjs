...
(async()=>{try {
  ...
  assert.equal(delta.delta,true); assert.equal(delta.signal.length,0); assert.equal(delta.traffic.length,0);
  const next={...delta,serverTime:first.serverTime+1000,signal:[{time:first.cursor.signal+1,status:'ok',rsrpLTE:-90,rsrpNR:null,sinrLTE:15,sinrNR:null,temperature:42}],traffic:[{time:first.cursor.traffic+1,download:1024,upload:512}]};
  ...
  const merged=merge(first,next); assert.equal(merged.signal.length,first.signal.length+1); assert.equal(merged.traffic.length,first.traffic.length+1);
  assert.equal(merge(merged,next).signal.length,merged.signal.length);
  assert.equal(merge(merged,next).traffic.length,merged.traffic.length);
  const expired=merge(merged,{...next,serverTime:next.serverTime+301000,signal:[],traffic:[]});
  assert.equal(expired.signal.length,0); assert.equal(expired.traffic.length,0);
